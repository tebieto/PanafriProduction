 ...
 <buyer-not :id="{{auth::id()}}"></buyer-not>
	<!--Begin header class DIV-->
	 ...
	 <!--End of Creating Menu Icon from scatch with Css-->
		...
		<h3> Transactions . Chats <a class="learn-more" style="color:blue;" @click="pendingUrl">Pending</a> <a class="learn-more" style="color:blue;" @click="transactionsUrl()">Active</a>  <a class="learn-more" style="color:blue;">Completed</a> </h3>		
		</div>
		
		<div class="suggestions">
		<span v-for="tracker in buyerChatTrackers">
		<buyer-chat :id="tracker.id" auth="{{auth::id()}}" :delivery="tracker.delivery" :seller="tracker.seller_id" :buyer="tracker.buyer_id":shop="tracker.shop_id"></buyer-chat>
		</span>
	...
	<!--End main-categories class div-->
		...
		@if (auth::check())
		<div class="pending-transaction-button" @click="transactionsUrl()">
		<div>
		
		<img class="pending-transaction-avatar"  width="30px" height="auto" src="{{Storage::url('public/icons/transactions.png')}}"/>
