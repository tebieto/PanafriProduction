...
<div class="content-title">Panafri Categories </div>

<!-- Products right here -->


<div class="no-result" v-if="categories.length==0">No categories at the moment</div>
...
<div class="product-name" v-if="product.type==1">@{{product.name.slice(0, 25).toUpperCase()}} PRODUCTS</div>
<div class="product-name" v-else>@{{product.name.slice(0, 25).toUpperCase()}} SERVICES</div>
...
<button v-if="partnerProduct==product.id"  @click="adminDeleteCategory(product.id)" id="yes-button">Yes</button>  <button v-if="partnerProduct==product.id"  @click="removeContact()" id="no-button">No</button>
<div class="call-partner" v-if="partnerProduct==product.id">
<div class="sure">Are you sure you want to delete this category?</div>
</div>
