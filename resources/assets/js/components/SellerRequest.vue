...
<div class="transactions" v-if="buyerDetails.length>0 && trackedTransactions.length>0">
		...
		<div style="margin-top:-50px; margin-left:80px;"><span class="user-name">{{buyerDetails[0].fname}} {{buyerDetails[0].lname}}</span> as Buyer </div>
		...
		<div style="height:150px; overflow-y:auto;">
		<h4 style="margin-left:10px;">Total = N {{total}} <br> Delivery Fee(N): <input type="number" style="width:80px;" value="0" v-model="delivery"/><br>Total + Delivery = N {{totalAndDelivery}}</h4>
		<span v-if="cancelled==false && accepted==false">
		<button style="margin-left:10px; border:none; color:#fff; background:green; font-weight:bold;" @click="acceptRequest">Accept</button>  <button style="margin-left:10px; border:none; color:#fff; background:red; font-weight:bold;" @click="cancelTransaction()">Decline</button> <button style="margin-left:10px; border:none; color:#fff; background:black; font-weight:bold;">Report</button>
		</span>
...
acceptRequest() {
this.accepted=  true
axios.get('/accept/transaction/' + this.id + '/' + this.delivery + '/' + this.buyer).then(response=>{
